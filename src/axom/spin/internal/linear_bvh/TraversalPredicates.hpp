...
public:
  ...
  AXOM_HOST_DEVICE static inline bool pointInLeftBin(
   ...
   */
  template <typename PointType>
  AXOM_HOST_DEVICE static inline bool pointInRightBin(
    const PointType& point,
  ...
  AXOM_HOST_DEVICE static inline bool rayIntersectsLeftBin(
    const RayType& r,
    const vec4_t<FloatType>& s1,
    const vec4_t<FloatType>& s2,
    FloatType TOL = numerics::floating_point_limits<FloatType>::epsilon()) noexcept;
   ...
   */
  template <typename RayType>
  AXOM_HOST_DEVICE static inline bool rayIntersectsRightBin(
    const RayType& r,
    const vec4_t<FloatType>& s2,
  ...
  AXOM_HOST_DEVICE static inline bool boundingBoxIntersectsLeftBin(
   ...
   */
  template <typename BoundingBoxType>
  AXOM_HOST_DEVICE static inline bool boundingBoxIntersectsRightBin(
    const BoundingBoxType& b,
    const vec4_t<FloatType>& s2,
    const vec4_t<FloatType>& s3) noexcept;
...
public:
  template <typename PointType>
  AXOM_HOST_DEVICE static inline bool pointInLeftBin(const PointType& point,
                                                     const vec4_t<FloatType>& s1,
                                                     const vec4_t<FloatType>& s2) noexcept
  {
    ...
    const vec4_t<FloatType>& s2) noexcept
  {
    ...
    return primal::detail::intersect_bounding_box(box_xmin,
                                                  box_xmax,
                                                  box_ymin,
                                                  box_ymax,
                                                  bin_xmin,
...
public:
    ...
    FloatType TOL = numerics::floating_point_limits<FloatType>::epsilon()) noexcept
  {
    ...
    //       candidate bins in a priority queue.
    FloatType t = 0.0;
    return primal::detail::intersect_ray(x0,
                                         nx,
                                         y0,
                                         ny,
                                         z0,
