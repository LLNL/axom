...
void driveUniformGrid()
{
  ...
  printPairs(" clashes found by naive algorithm for non-intersecting triangles:", naiveclashesNonIntersect);
  printPairs(" clashes found by naive algorithm for intersecting triangles:", naiveclashesIntersect);
  std::cout << std::endl;
}
...
void try_intersect(umpire::Allocator allocator)
{
  ...
  PointType * p = axom::allocate <PointType> (5, allocator);
  int * numIntersect = axom::allocate <int> (1, allocator);
  ...
  using ATOMIC_POL_SEQ = RAJA::seq_atomic;
  ...
  RAJA::kernel<KERNEL_EXEC_POL_SEQ>( RAJA::make_tuple(col_range, row_range),
    ...
    if (row > col)
    {
      ...
      {
        RAJA::atomicAdd<ATOMIC_POL_SEQ>(numIntersect, 1);
      }
    }
  });
  
  // Print size
  std::cout << "Number of intersections is " << numIntersect[0] << "(Expected was 5)" << std::endl;

  // Allocation to hold intersection pairs and counter to know where to store
  int * intersections = axom::allocate <int> (numIntersect[0] * 2, allocator);
  ...
  RAJA::kernel<KERNEL_EXEC_POL_SEQ>( RAJA::make_tuple(col_range, row_range),
    ...
    if (row > col)
    {
      if (intersect (tri[row], tri[col]))
      {
        intersections[counter[0]] = row;
        intersections[counter[0] + 1] = col;
        RAJA::atomicAdd<ATOMIC_POL_SEQ>(counter, 2);
  ...
  for (int i = 0 ; i < numIntersect[0] * 2 ; i = i + 2)
  {
