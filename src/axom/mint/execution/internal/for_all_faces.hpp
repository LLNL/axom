...
inline void for_all_I_faces( xargs::ij,
...
//------------------------------------------------------------------------------
template < typename ExecPolicy, typename KernelType >
inline void for_all_J_faces( xargs::ij,
                             const StructuredMesh& m,
                             KernelType&& kernel )
{
  SLIC_ERROR_IF( m.getDimension() != 2, "Mesh must be 2D." );
                             ...
                             KernelType&& kernel )
{
  ...
  const IndexType numIJFaces = m.getTotalNumFaces( I_DIRECTION )
                             + m.getTotalNumFaces( J_DIRECTION );
  const IndexType ICellResolution = m.getCellResolution( I_DIRECTION );
...
//------------------------------------------------------------------------------
template < typename ExecPolicy, typename KernelType >
inline void for_all_faces_impl( xargs::index,
                                const Mesh& m,
                                KernelType&& kernel )
{
                                ...
                                KernelType&& kernel )
{
  ...
  if ( dimension == 2 )
  {
    ...
    helpers::for_all_J_faces< ExecPolicy >( xargs::ij(), m,
  ...
  else
  {
    ...
    helpers::for_all_J_faces< ExecPolicy >( xargs::ijk(), m,
      AXOM_LAMBDA( IndexType faceID, IndexType AXOM_NOT_USED(i), IndexType j,
                   IndexType k )
      {
        const IndexType shiftedID = faceID - numIFaces;
    ...
    helpers::for_all_K_faces< ExecPolicy >( xargs::ijk(), m,
      AXOM_LAMBDA( IndexType faceID, IndexType AXOM_NOT_USED(i), IndexType j,
                   IndexType k )
      {
