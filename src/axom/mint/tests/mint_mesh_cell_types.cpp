...
TEST( mint_mesh_cell_types, check_cell_types )
{
  ...
  const mint::IndexType
    ...
    { 0, 1, 2,      // face 0 for PRISM
      0, 2, 5, 3,   // face 1
      0, 3, 4, 1,   // face 2
      1, 4, 5, 2,   // face 3
      3, 5, 4 },    // face 4

    { 0, 3, 2, 1,   // face 0 for PYRAMID
      0, 1, 4,      // face 1
      1, 2, 4,      // face 2
      2, 3, 4,      // face 3
      3, 0, 4 },    // face 4

    { 0, 1,         // face 0 for QUAD9
      1, 2,         // face 1
      2, 3,         // face 2
      3, 0 },       // face 3
  ...
  for ( int i=0 ; i < mint::NUM_CELL_TYPES ; ++i )
  {
    ...
    for (int faceidx = 0; faceidx < num_faces[ i ]; ++faceidx)
    {
      ...
      EXPECT_EQ( mint::cell_info[ i ].face_nodecount[ faceidx ],
                 face_ncount );
      EXPECT_EQ( mint::cell_info[ i ].face_types[ faceidx ],
                 face_types[ i ][ faceidx ] );
