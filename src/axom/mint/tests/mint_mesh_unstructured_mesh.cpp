 ...
 * \param [in] i the tuple index.
 */
void setNewFieldTuple( FieldVariable< double >* fv, IndexType n_tuples,
                       IndexType i )
{
  EXPECT_EQ( n_tuples, fv->getNumTuples() );
                         ...
                         int dimension )
{
  ...
  case 2:
    source_mesh =
      new UniformMesh( lo, hi, resolution,
...
void append_cell_single( UnstructuredMesh< SINGLE_SHAPE >* mesh,
                         IndexType n_cells )
{
...
void append_cell_single( UnstructuredMesh< MIXED_SHAPE >* mesh,
                         IndexType n_cells )
{
  IndexType cur_n_cells = mesh->getNumberOfCells();
...
void append_cell_multiple( UnstructuredMesh< SINGLE_SHAPE >* mesh,
                           IndexType n_cells )
{
  IndexType cur_n_cells = mesh->getNumberOfCells();
  IndexType cur_connec_size = mesh->getCellNodesSize();
...
void append_cell_multiple( UnstructuredMesh< MIXED_SHAPE >* mesh,
                           IndexType n_cells )
{
  IndexType cur_n_cells = mesh->getNumberOfCells();
  IndexType cur_connec_size = mesh->getCellNodesSize();
 ...
 */
template < Topology TOPO >
void insert_node_structs( UnstructuredMesh< TOPO >* mesh, IndexType n_nodes,
                          IndexType pos, IndexType final_pos, bool update=true )
{
  const int ndims = mesh->getDimension();
  IndexType cur_n_nodes = mesh->getNumberOfNodes();
  FieldVariable< double >* fv = getFieldVar( mesh, NODE_CENTERED );
 ...
 */
template < Topology TOPO >
void insert_node_arrays( UnstructuredMesh< TOPO >* mesh, IndexType n_nodes,
                         IndexType pos, IndexType final_pos, bool update=true )
{
  const int ndims = mesh->getDimension();
  IndexType cur_n_nodes = mesh->getNumberOfNodes();
 ...
 */
void check_faces( UnstructuredMesh< SINGLE_SHAPE >* d_mesh,
                  UniformMesh * s_mesh )
{
  ...
  for (IndexType c = 0; c < cellcount; ++c)
  {
    ...
    std::string errMesg;
    bool facesMatch = check_cellFaceNodeType(s_mesh, d_mesh, s_facecount,
...
TEST( mint_mesh_unstructured_mesh, check_face_connectivity )
{
  ...
  constexpr int SIZE_STEPS = 3;
  constexpr IndexType SIZE_FACTOR[] = { 1, 3, 7 };
  ...
  for (int isize = 0; isize < SIZE_STEPS; ++isize)
  {
    ...
    for (int idim = 0; idim < DIM_STEPS; ++idim)
    {
      ...
      internal::createMeshesForFace( test_mesh, source_mesh,
