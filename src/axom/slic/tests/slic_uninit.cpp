...
{
	testInit("createLogger", 
		[]() { axom::slic::createLogger("foo"); });
	testInit("activateLogger", 
		[]() { (void)axom::slic::activateLogger("foo"); });
	testInit("getActiveLoggerName", 
		[]() { (void)axom::slic::getActiveLoggerName(); });
	testInit("setLoggingMsgLevel", 
		[]() { axom::slic::setLoggingMsgLevel(axom::slic::message::Info); });
	testInit("getLoggingMsgLevel",
		[]() { axom::slic::getLoggingMsgLevel(); });

	testInit("setAbortOnError",
		[]() { axom::slic::setAbortOnError(true); });
	testInit("enableAbortOnError",
		[]() { axom::slic::enableAbortOnError(); });
	testInit("disableAbortOnError",
		[]() { axom::slic::disableAbortOnError(); });
	testInit("isAbortOnErrorsEnabled",
		[]() { (void)axom::slic::isAbortOnErrorsEnabled(); });

	testInit("setAbortOnWarning",
		[]() { axom::slic::setAbortOnWarning(true); });
	testInit("enableAbortOnWarning",
...
TEST(slic_uninit, log_methods)
{
	...
	testInit("logMessage_B",
		[lvl]() { axom::slic::logMessage(lvl, "another info message", "tagB"); });
	testInit("logMessage_C",
		[lvl, fname, line]() { axom::slic::logMessage(lvl, "info msg", fname, line); });
	testInit("logMessage_D",
		[lvl, fname, line]() { axom::slic::logMessage(lvl, "info msg", "tagD", fname, line); });

	testInit("logErrorMessage",
		[fname, line]() { axom::slic::logErrorMessage("an error message", fname, line); });
	testInit("logWarningMessage",
		[fname, line]() { axom::slic::logWarningMessage("a warning message", fname, line); });
}
