...
public:
  ...
  void split_bezier(T t,
                    BezierCurve< T, NDIMS >& c1,
                    BezierCurve< T, NDIMS>& c2) const
  {
    int ord = m_controlpoints.size()-1;
    ...
    for ( int i=0 ; i < NDIMS ; i++)
    {
      ...
      for ( int j=1 ; j <= ord ; j++)
      {
        for ( int k=0 ; k <= ord-j ; k++)
        {
          dCarray[i*((ord+1))+k]=(1-t)*dCarray[i*((ord+1))+k]
                                  + t*dCarray[i*((ord+1))+k+1];
        }
  ...
  bool is_linear(double tol) const
  {
    ...
    double d1=0.0;
    for (int i=1 ; i<ord1 ; i++) // line defined by endpoints; only add interior
    {
