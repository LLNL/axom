...
public:
  ...
  std::vector<BezierCurve<T,NDIMS>> getEdges() const
  {
    return m_edges;
  }
  ...
  bool isClosed() const
  {
    ...
    else
    {
      for (int p=0; p<NDIMS; ++p)
      {
        ...
        {
          if (!axom::utilities::isNearlyEqual(m_edges[i][m_edges[i].getOrder()][p],m_edges[i+1][0][p]))
          {
            return false;
          }
        } 
        if (!axom::utilities::isNearlyEqual(m_edges[ngon-1][m_edges[ngon-1].getOrder()][p],m_edges[0][0][p]))
        {
          return false;
        }
      }
