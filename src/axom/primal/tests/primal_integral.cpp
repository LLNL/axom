...
TEST(primal_integral, evaluate_area_integral)
{
  ...
  Bezier square1(squarenodes1, 1);

  Point2D squarenodes2[] = {Point2D {1.0, 0.0}, Point2D {1.0, 1.0}};
  Bezier square2(squarenodes2, 1);

  Point2D squarenodes3[] = {Point2D {1.0, 1.0}, Point2D {0.0, 1.0}};
  Bezier square3(squarenodes3, 1);

  Point2D squarenodes4[] = {Point2D {0.0, 1.0}, Point2D {0.0, 0.0}};
  Bezier square4(squarenodes4, 1);

  Bezier square_edges[] = {square1, square2, square3, square4};
  CPolygon square(square_edges, 4);

  EXPECT_NEAR(evaluate_area_integral(square, const_integrand, 10), 1.0, abs_tol);
}
...
TEST(primal_integral, evaluate_line_integral_vector)
{
  ...
  EXPECT_NEAR(
    evaluate_vector_line_integral(parabola_shape, area_field, npts, tot_npts),
    8.0 / 3.0,
    abs_tol);

  // This vector field is conservative, so it should evaluate to zero
  EXPECT_NEAR(evaluate_vector_line_integral(parabola_shape,
                                            conservative_field,
                                            npts,
                                            tot_npts),
              0.0,
  ...
  EXPECT_NEAR(
    evaluate_vector_line_integral(parabola_shape, winding_field, npts, tot_npts),
    1.0,
    abs_tol);
...
TEST(primal_integral, evaluate_integral_rational)
{
  ...
  EXPECT_NEAR(evaluate_vector_line_integral(ellipse_arc, area_field, npts),
              M_PI * 2 * 1 / 4.0,
              abs_tol);
  EXPECT_NEAR(evaluate_vector_line_integral(quarter_ellipse,
                                            conservative_field,
                                            npts,
                                            tot_npts),
              0,
...
TEST(primal_integral, evaluate_integral_bezierpatch)
{
  using Point3D = primal::Point<double, 3>;
  using Vector3D = primal::Vector<double, 3>;
  using Bezier = primal::BezierCurve<double, 3>;
  ...
  Point3D rect_data[4] = {Point3D {0.0, 0.0, 0.0},
                          Point3D {0.0, 3.0, 0.0},
                          Point3D {2.0, 0.0, 0.0},
                          Point3D {2.0, 3.0, 0.0}};

  BPatch rect(rect_data, 1, 1);

  // Test area of square
  EXPECT_NEAR(evaluate_scalar_area_integral(rect, const_integrand, 10),
              6.0,
              abs_tol);

  // Test area of triangle
  rect(1, 1) = Point3D {2.0, 0.0, 0.0};
  EXPECT_NEAR(evaluate_scalar_area_integral(rect, const_integrand, 10),
              3.0,
              abs_tol);

  // Test surface area of a half sphere

  // clang-format off
  Point3D hemisphere_data[16] = {  
  ...
  BPatch hemisphere(hemisphere_data, hemisphere_weights, 3, 3);
