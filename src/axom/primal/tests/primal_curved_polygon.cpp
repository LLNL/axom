...
void checkMoments(const primal::CurvedPolygon<CoordType, DIM>& bPolygon,
                  const CoordType expArea, const primal::Point<CoordType,DIM>& expMoment,
                  double eps, double test_eps)
{
                  ...
                  const std::vector<int> orders)
{
   ...
   EXPECT_EQ(accumulate(orders.begin(),orders.end(), 0)+1,num_unique_control_points);
...
TEST( primal_curvedpolygon, split_edge)
{
  ...
  BezierCurveType bCurve2; BezierCurveType bCurve3;
...
TEST( primal_curvedpolygon, moments_triangle_degenerate)
{
  ...
  SLIC_INFO("Test checking CurvedPolygon degenerate triangle area computation.");
...
TEST( primal_curvedpolygon, moments_triangle_quadratic )
{
  ...
  SLIC_INFO("Test checking CurvedPolygon mixed order triangle moment computation.");
    ...
    for (int p=2; p<11; ++p)
    {
      ...
      for (int side=0; side<4; ++side)
      {
        for (int i=1; i<p; ++i)
        {
          switch (side)
          {  
            case 0: CP.insert(CP.begin()+i+(side*p),(PointType::make_point(0.0,1.0*i/p)));
                    break;
            case 1: CP.insert(CP.begin()+i+(side*p),(PointType::make_point(1.0*i/p,1.0)));
                    break;
            case 2: CP.insert(CP.begin()+i+(side*p),(PointType::make_point(1.0,1.0-(1.0*i/p))));
                    break;
            case 3: CP.insert(CP.begin()+i+(side*p),(PointType::make_point(1.0-(1.0*i/p),0.0)));
                    break;
          }
        }
        orders[side]+=1;
...
TEST( primal_curvedpolygon, intersection_triangle_linear)
{
  ...
  SLIC_INFO("Test intersecting two linear triangular CurvedPolygons (single region).");

  CurvedPolygonType bPolygon;
  EXPECT_EQ(0, bPolygon.numEdges());

  PointType controlPoints[order+1] = {
    PointType::make_point(0.6, 1.2),
    PointType::make_point(0.3, 2.0)
  };

  PointType controlPoints2[order+1] = {
    PointType::make_point( 0.3, 2.0),
    PointType::make_point( 0.0 , 1.6)
  };

  PointType controlPoints3[order+1] = {
    PointType::make_point( 0.0 , 1.6),
    PointType::make_point( 0.6, 1.2)
  };
  ...
  for (int i=0; i<DIM; ++i)
  {
    for (int j=0; j<=order; ++j)
    {
      for (int idxcurve=0; idxcurve< static_cast<int>(bPolygons3.size()); ++idxcurve)
      {
...
TEST( primal_curvedpolygon, intersection_triangle_quadratic)
{
  ...
  SLIC_INFO("Test intersecting two quadratic triangular CurvedPolygons (single region).");
...
TEST( primal_curvedpolygon, area_intersection_triangle_linear)
{
  ...
  SLIC_INFO("Test finding area of intersection two linear triangular CurvedPolygons (single region).");
...
TEST( primal_curvedpolygon, area_intersection_triangle_quadratic_two_regions)
{
  ...
  SLIC_INFO("Test intersecting two quadratic triangular CurvedPolygons (two regions).");
...
TEST( primal_curvedpolygon, area_intersection_triangle_inclusion)
{
  ...
  SLIC_INFO("Test intersecting two quadratic triangular CurvedPolygons (inclusion).");
