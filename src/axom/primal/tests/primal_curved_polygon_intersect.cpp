...
void checkIntersection(
    ...
    const double eps = 1e-15, const double test_eps = 1e-13)
{
  ...
  for (int i=0; i<DIM; ++i)
  {
    ...
    { 
      ...
      for (int k = 0; k < nEd; ++k)
      {
        ...
        for (int j = 0; j <= ord; ++j)
        {
          EXPECT_NEAR(expbPolygon[idxcurve][k][j][i],intersectionPolys[idxcurve][k][j][i],test_eps);
        }
      }
    }
  }
