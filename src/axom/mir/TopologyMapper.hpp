                                     ...
                                     double eps = 1.e-10)
{
  ...
  {
    axom::primal::Tetrahedron<T, 3> tet(shape1[0],
                                        shape1[1],
                                        shape1[2],
                                        shape1[3]);
                                     ...
                                     double eps = 1.e-10)
{
  ...
  if(id == axom::mir::views::Tet_ShapeID)
  {
    axom::primal::Tetrahedron<T, 3> tet(shape2[0],
                                        shape2[1],
                                        shape2[2],
                                        shape2[3]);
 ...
 *       types.
 */
template <typename ExecSpace, typename SrcTopologyView, typename SrcCoordsetView, typename TargetTopologyView, typename TargetCoordsetView>
class TopologyMapper
{
public:
  static_assert(SrcCoordsetView::dimension() == TargetCoordsetView::dimension(), "coordset dimension mismatch");
  ...
  struct PrimalAdaptor
  {
    ...
    using Polygon = axom::primal::Polygon<value_type, CoordsetView::dimension(), axom::primal::PolygonArray::Static>;
    ...
    AXOM_HOST_DEVICE typename std::enable_if<TDIM == 3 &&
                                             std::is_same<ShapeType, axom::mir::views::TetShape<typename ShapeType::ConnectivityStorage>>::value,
                                             Tetrahedron>::type getShape(axom::IndexType zi) const
    {
      const auto zone = m_topoView.zone(zi);
    ...
    AXOM_HOST_DEVICE typename std::enable_if<TDIM == 3 &&
                                             ...
                                             Hexahedron>::type getShape(axom::IndexType zi) const
    {
      const auto zone = m_topoView.zone(zi);
    ...
    AXOM_HOST_DEVICE typename std::enable_if<(TDIM == 3) &&
                                             ...
                                             detail::VariableShape<value_type, 3> >::type getShape(axom::IndexType zi) const
    {
      const auto zone = m_topoView.zone(zi);
   ...
   */
  TopologyMapper(const SrcTopologyView &srcTopoView, const SrcCoordsetView &srcCoordsetView,
                 const TargetTopologyView &targetTopoView, const TargetCoordsetView &targetCoordsetView) :
               ...
               conduit::Node &n_targetMesh)
  {
    ...
    const std::string targetTopologyName = n_options[TARGET_TOPOLOGY_NAME].as_string();
    const std::string targetMatsetName = n_options[TARGET_MATSET_NAME].as_string();
    ...
    axom::mir::SelectedZones<ExecSpace> srcSelection(srcView.numZones(), n_options, SRC_SELECTED_ZONES);
    ...
    axom::for_all<ExecSpace>(nSrcZones, AXOM_LAMBDA(axom::IndexType index)
    {
    ...
    const auto srcMatIds = bputils::make_array_view<std::int64_t>(n_matset["material_ids"]);
    const auto bvh_device = bvh.getTraverser();
    axom::mir::SelectedZones<ExecSpace> targetSelection(targetView.numZones(), n_options, TARGET_SELECTED_ZONES);
    targetSelection.setSorted(false);
    const auto targetSelectionView = targetSelection.view();
    axom::for_all<ExecSpace>(targetSelectionView.size(), AXOM_LAMBDA(axom::IndexType index)
    {
...
#if defined(AXOM_DEBUG_TOPOLOGY_MAPPER) && !defined(AXOM_DEVICE_CODE)
      std::cout << "-------------------------------\ntarget zone " << zi << ": " << targetShape << ", bbox=" << targetBBox << std::endl;
#endif
      // Get the area or volume of the target shape (depends on the dimension).
      double targetAmount = 1.;
      if constexpr(TargetCoordsetView::dimension() == 2)
      {
        targetAmount = targetShape.area();
      }
      else
      {
        targetAmount = targetShape.volume();
      }
                                    ...
                                    const std::int32_t* leafNodes)
      {
        ...
        double srcOverlapsTarget = detail::shapeOverlap(srcShape, targetShape, eps);
        ...
        if(srcOverlapsTarget > 0.)
        {
          ...
          for(int m = 0; m < nmats; m++)
          {
            ...
            {
#if defined(AXOM_DEBUG_TOPOLOGY_MAPPER) && !defined(AXOM_DEVICE_CODE)
              std::cout << "\t\tAdded new slot " << m << ", mat=" << mat << ", vf=" << vf << std::endl;
#endif
              matids[m] = mat;
              vfs[m] = vf;
              sizes[zi]++;
    ...
    axom::for_all<ExecSpace>(targetView.numZones(), AXOM_LAMBDA(axom::IndexType zi)
    {
