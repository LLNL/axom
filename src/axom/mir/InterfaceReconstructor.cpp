    ...
    for (int eID = 0; eID < intermediateMesh.m_elems.size(); ++eID)
    {
      ...
      for (int matID = 0; matID < intermediateMesh.m_numMaterials; ++matID)
      {
        ...
        for (unsigned long vID = 0; vID < elementVertices.size(); ++vID)
        {
          ...
          materialVertexVF.push_back( intermediateMesh.m_materialVolumeFractionsVertex[matID][originalVID] );
        }
...
void  InterfaceReconstructor::generateCleanCells(mir::Shape shapeType,
                                                 ...
                                                 CellData& out_cellData)
{
  ...
  else
  {
    ...
    for (auto itr = newElements.begin(); itr != newElements.end(); itr++)
    {
      out_cellData.m_mapData.m_shapeTypes[itr->first] = mir::utilities::determineElementShapeType(shapeType, itr->second.size());
    }
