...
void CellData::mergeCell(const CellData& cellToMerge)
{
  ...
  {
    m_topology.m_evInds.push_back(cellToMerge.m_topology.m_evInds[i] +
                                  vertexIndexOffset);
  }

  for(unsigned long i = 1; i < cellToMerge.m_topology.m_evBegins.size(); ++i)
  {
    m_topology.m_evBegins.push_back(cellToMerge.m_topology.m_evBegins[i] +
                                    evBeginsOffset);
  }

  for(unsigned long i = 0; i < cellToMerge.m_topology.m_veInds.size(); ++i)
  {
    m_topology.m_veInds.push_back(cellToMerge.m_topology.m_veInds[i] +
                                  elementIndexOffset);
  }

  for(unsigned long i = 1; i < cellToMerge.m_topology.m_veBegins.size(); ++i)
  {
    m_topology.m_veBegins.push_back(cellToMerge.m_topology.m_veBegins[i] +
                                    veBeginsOffset);
  }

  // Merge the vertex positions
  for(unsigned long i = 0; i < cellToMerge.m_mapData.m_vertexPositions.size();
      ++i)
  {
    m_mapData.m_vertexPositions.push_back(
      cellToMerge.m_mapData.m_vertexPositions[i]);
  }

  // Merge the vertex volume fractions
  for(unsigned long matID = 0; matID < m_mapData.m_vertexVolumeFractions.size();
      ++matID)
  {
    for(unsigned long vID = 0;
        vID < cellToMerge.m_mapData.m_vertexVolumeFractions[matID].size();
        ++vID)
    {
  ...
  for(unsigned long i = 0;
      i < cellToMerge.m_mapData.m_elementDominantMaterials.size();
      ++i)
  {
  ...
  {
    m_mapData.m_elementParents.push_back(
      cellToMerge.m_mapData.m_elementParents[i]);
  }
