...
{
  using Indexing2D = views::StridedStructuredIndexing<int, 2>;
  using LogicalIndex = typename views::StridedStructuredIndexing<int, 2>::LogicalIndex;
  ...
  const auto index2_2 = indexing.LogicalIndexToIndex(LogicalIndex{2, 2});
  EXPECT_EQ(index2_2, 14);
  LogicalIndex logical = indexing.IndexToLogicalIndex(index2_2);
  EXPECT_EQ(logical, LogicalIndex{2,2});
