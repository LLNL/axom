...
TEST(mir_clipping_case, all_triangle_cases)
{
  ...
  for(auto actualClippingCase = 0; actualClippingCase < numCases;
      ++actualClippingCase)
  {
    ...
    unsigned int computedClippingCase =
      clipper.determineClippingCase(shape, matOneVF, matTwoVF);
...
TEST(mir_clipping_case, all_tetrahedron_cases)
{
  ...
  for(unsigned int actualClippingCase = 0;
      actualClippingCase < (unsigned int)numCases;
      ++actualClippingCase)
  {
    std::vector<axom::float64> matOneVF;
    std::vector<axom::float64> matTwoVF;
...
TEST(mir_clipping_place, clip_edge_when_mat_one_dominates)
{
  ...
  mir::CellClipper clipper;
  axom::float64 tValue = clipper.computeTValueOnEdge(vfMatOneVertexOne,
                                                     vfMatTwoVertexOne,
                                                     vfMatOneVertexTwo,
                                                     vfMatTwoVertexTwo);
...
TEST(clipping_table_mesh_generation, triangle_meshes)
{
  ...
  for(auto actualClippingCase = 0u;
      actualClippingCase < mir::triangleClipTableVec.size();
      ++actualClippingCase)
  {
    // Initialize the mesh
    int numElements = 1;
    ...
    std::vector<std::vector<axom::float64>> vertexVF = {matOneVF, matTwoVF};

    std::vector<std::vector<axom::float64>> elementVF = {{0.5}, {0.5}};

    std::vector<mir::Point2> points = {mir::Point2::make_point(0.5, 0.717),
    ...
    std::string fileName = "mir_clippingcase_triangle_" +
      std::to_string(actualClippingCase) + "_" + bitString + ".vtk";
...
TEST(clipping_table_mesh_generation, quad_meshes)
{
  ...
  for(unsigned int actualClippingCase = 0;
      actualClippingCase < mir::quadClipTableVec.size();
      ++actualClippingCase)
  {
    // Initialize the mesh
    int numElements = 1;
    ...
    std::vector<mir::Point2> points = {mir::Point2::make_point(0.0, 1.0),
                                       mir::Point2::make_point(0.0, 0.0),
                                       mir::Point2::make_point(1.0, 0.0),
    ...
    // Write out the processed mesh
    std::string dirName = std::string(AXOM_BIN_DIR) + "/meshes";
    std::string fileName = "mir_clippingcase_quad_" +
      std::to_string(actualClippingCase) + "_" + bitString + ".vtk";
    outputMesh.writeMeshToFile(dirName, fileName, "/");
  }
}
...
TEST(clipping_table_mesh_generation, tetrahedron_meshes)
{
  ...
  for(unsigned int actualClippingCase = 0;
      actualClippingCase < mir::tetrahedronClipTableVec.size();
      ++actualClippingCase)
  {
    // Initialize the mesh
    int numElements = 1;
    ...
    // Write out the processed mesh
    std::string dirName = std::string(AXOM_BIN_DIR) + "/meshes";
    std::string fileName = "mir_clippingcase_tetrahedron_" +
      std::to_string(actualClippingCase) + "_" + bitString + ".vtk";
    outputMesh.writeMeshToFile(dirName, fileName, "/");
  }
}
...
TEST(clipping_table_mesh_generation, pyramid_meshes)
{
  ...
  for(unsigned int actualClippingCase = 0;
      actualClippingCase < mir::pyramidClipTableVec.size();
      ++actualClippingCase)
  {
    {
      // Initialize the mesh
      ...
      // Write out the processed mesh
      std::string dirName = std::string(AXOM_BIN_DIR) + "/meshes";
      std::string fileName = "mir_clippingcase_pyramid_" +
        std::to_string(actualClippingCase) + "_" + bitString + ".vtk";
      outputMesh.writeMeshToFile(dirName, fileName, "/");
    }
  }
...
TEST(clipping_table_mesh_generation, triangular_prism_meshes)
{
  ...
  for(unsigned int actualClippingCase = 0;
      actualClippingCase < mir::triangularPrismClipTableVec.size();
      ++actualClippingCase)
  {
    // Initialize the mesh
    int numElements = 1;
    ...
    std::vector<mir::Point2> points = {mir::Point2::make_point(0.5, 0.717, 0.0),
                                       mir::Point2::make_point(0.0, 0.0, 0.0),
                                       mir::Point2::make_point(1.0, 0.0, 0.0),
    ...
    // Write out the processed mesh
    std::string dirName = std::string(AXOM_BIN_DIR) + "/meshes";
    std::string fileName = "mir_clippingcase_triangular_prism_" +
      std::to_string(actualClippingCase) + "_" + bitString + ".vtk";
    outputMesh.writeMeshToFile(dirName, fileName, "/");
  }
}
...
TEST(clipping_table_mesh_generation, hexahedron_meshes)
{
  ...
  for(unsigned int actualClippingCase = 0;
      actualClippingCase < mir::hexahedronClipTableVec.size();
      ++actualClippingCase)
  {
    // Initialize the mesh
    int numElements = 1;
    ...
    // Write out the processed mesh
    std::string dirName = std::string(AXOM_BIN_DIR) + "/meshes";
    std::string fileName = "mir_clippingcase_hexahedron_" +
      std::to_string(actualClippingCase) + "_" + bitString + ".vtk";
    outputMesh.writeMeshToFile(dirName, fileName, "/");
  }
}
