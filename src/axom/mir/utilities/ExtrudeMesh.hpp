...
{
public:
  static_assert(CoordsetView::dimension() == 2, "Coordset must contain 2D coordinates.");
   ...
   */
  ExtrudeMesh(const TopologyView &topoView, const CoordsetView &coordsetView) : m_topoView(topoView),
  ...
  void execute(const conduit::Node &n_mesh, const conduit::Node &n_options, conduit::Node &n_output) const
  {
    ...
    const std::string srcTopoName = n_options.has_child("topologyName") ? n_options["topologyName"].as_string() : "main";
    ...
    const std::string outputTopoName = n_options.has_child("outputTopologyName") ? n_options["outputTopologyName"].as_string() : srcTopoName;
    const std::string outputCoordsetName = n_options.has_child("outputCoordsetName") ? n_options["outputCoordsetName"].as_string() : srcCoordsetName;
    ...
    axom::for_all<ExecSpace>(m_topoView.numberOfZones(), AXOM_LAMBDA(axom::IndexType zi)
    {
    ...
    const axom::IndexType totalZones = static_cast<axom::IndexType>(nz - 1) * m_topoView.numberOfZones();
    const axom::IndexType totalNodes = static_cast<axom::IndexType>(nz) * m_coordsetView.numberOfNodes();
    ...
    const value_type z0 = n_options.has_child("z0") ? n_options["z0"].to_double() : 0.;
    const value_type z1 = n_options.has_child("z1") ? n_options["z1"].to_double() : (nz - 1);
    for(int z = 0; z < nz; z++)
    {
      auto tz = static_cast<value_type>(z) / static_cast<value_type>(nz - 1);
      value_type zc = z0 + tz * (z1 - z0);
      axom::for_all<ExecSpace>(coordsetView.numberOfNodes(), AXOM_LAMBDA(int srcNodeIndex)
      {
    ...
    {
      axom::for_all<ExecSpace>(topoView.numberOfZones(), AXOM_LAMBDA(axom::IndexType zi)
      {
        const auto zone = topoView.zone(zi);
    ...
    if(!matsetName.empty())
    {
      ...
      std::string outputMatsetName = n_options.has_child("outputMatsetName") ? n_options["outputMatsetName"].as_string() : matsetName;
...
private:
   ...
   */
  void extrudeMatset(const conduit::Node &n_srcMatset, conduit::Node &n_outputMatset, const std::string &outputTopoName, int nz) const
  {
    namespace bputils = axom::mir::utilities::blueprint;
    ...
    n_indices.set(conduit::DataType(
      n_src_indices.dtype().id(),
      n_src_indices.dtype().number_of_elements() * (nz - 1)));
    n_sizes.set(conduit::DataType(
      n_src_sizes.dtype().id(),
      n_src_sizes.dtype().number_of_elements() * (nz - 1)));
    n_offsets.set(conduit::DataType(
      n_src_offsets.dtype().id(),
    ...
    axom::mir::views::FloatNode_to_ArrayView_same(n_src_volume_fractions, n_volume_fractions,
      [&](auto srcVolumeFractionsView, auto volumeFractionsView)
    {
      axom::mir::views::IndexNode_to_ArrayView_same(
        n_src_material_ids, n_src_indices, n_src_sizes, n_src_offsets,
        n_material_ids, n_indices, n_sizes, n_offsets,
        [&](auto srcMaterialIdsView, auto srcIndicesView, auto srcSizesView, auto srcOffsetsView,
            auto materialIdsView, auto indicesView, auto sizesView, auto offsetsView)
      {
        for(int z = 0; z < nz - 1; z++)
        {
          axom::for_all<ExecSpace>(nzones, AXOM_LAMBDA(axom::IndexType zi)
          {
            ...
            for(int i = 0; i < srcSizesView[offset]; i++)
            {
