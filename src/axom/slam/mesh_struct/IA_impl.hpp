...
template<unsigned int S, unsigned int D, typename P>
typename IAMesh<S,D,P>::ElementAndFaceIdxType
IAMesh<S,D,P>::ElemNbrFinder( V2EMapType & vertpair_to_elem_map,
                              IndexType element_i, IndexType side_i )
{
  typedef std::pair<IndexListType, ElementAndFaceIdxType>      N2ZPairType;
  ...
  map_ret2 = vertpair_to_elem_map.insert( N2ZPairType(vlist, zs_pair) );
  if(!map_ret2.second ) //this pair is in the map already
  {
...
template <unsigned int S, unsigned int D, typename P>
void IAMesh<S,D,P>::print_all() const {
  SLIC_INFO(
    "size of        "
    <<" vertex_set:" << vertex_set.size()
    <<" element_set:"<< element_set.size()
    <<" ev:"<< ev_rel.size()
    <<" ve:"<< ve_rel.size()
    <<" ee:"<< ee_rel.size()
    <<" map:"<< vcoord_map.size());
  ...
  SLIC_INFO(
    ...
    <<" map:"<< vcoord_map.numberOfValidEntries());

  //print out the sets, relation, and map
  std::cout<<"element_set:"<<element_set.numberOfValidEntries()
           <<"/"<< element_set.size();

  for(int i=0 ; i<element_set.size() ; i++)
  {
    std::cout<< " " << i << ":" << element_set[i];
  }
  std::cout<<std::endl;

  std::cout<<"vertex_set:"<<vertex_set.numberOfValidEntries()
           <<"/"<< vertex_set.size();

  for(int i=0 ; i<vertex_set.size() ; i++)
  {
    std::cout<< " " << i << ":" << vertex_set[i];
  }
  std::cout<<std::endl;

  std::cout<<"ev_rel:"<<ev_rel.numberOfValidEntries() <<"/"<<ev_rel.size();
  for(int i=0 ; i<ev_rel.size() ; i++)
  {
    std::cout<< " " << i << ":";
    for(int j=0 ; j<ev_rel[i].size() ; j++)
    {
      std::cout<< ev_rel[i][j];
      if(j<VERTS_PER_ELEM-1)
        std::cout<<",";
    }
  }
  std::cout<<std::endl;

  std::cout<<"ve_rel:"<<ve_rel.numberOfValidEntries()<<"/"<<ve_rel.size();
  for(int i=0 ; i<ve_rel.size() ; i++)
  {
    std::cout<< " " << i << ":" << ve_rel[i][0];
  }
  std::cout<<std::endl;

  std::cout<<"ee_rel:"<<ee_rel.numberOfValidEntries()<<"/"<<ee_rel.size();
  for(int i=0 ; i<ee_rel.size() ; i++)
  {
    std::cout<< " " << i << ":";
  ...
  std::cout<<"vertex coord: "<<vcoord_map.numberOfValidEntries()
           <<"/"<< vcoord_map.size();

  for(int i=0 ; i<vcoord_map.size() ; i++)
  {
    std::cout<< " " << i << ":";
    ...
    {
      std::cout<< vcoord_map[i][j];
      if(j<COORDS_PER_VERT-1)
        std::cout<<",";
    }
  }
  std::cout<<std::endl;
...
template<unsigned int S, unsigned int D, typename P>
IAMesh<S,D,P>::IAMesh(const IAMesh& m)
{
  operator=(m);
}

template<unsigned int S, unsigned int D, typename P>
IAMesh<S,D,P>& IAMesh<S,D,P>::operator= (const IAMesh & m)
{
  if ( &m != this)
  {
    vertex_set = m.vertex_set;
    element_set = m.element_set;
    ev_rel = ElementToVertexRelation(&element_set, &vertex_set);
    ve_rel = VertexToOneElementRelation(&vertex_set, &element_set);
    ee_rel = ElementToElementRelation(&element_set, &element_set);
...
template<unsigned int S, unsigned int D, typename P>
IAMesh<S,D,P>::IAMesh(std::vector<double>& points, std::vector<int>& tri) :
  ...
  vcoord_map(&vertex_set)
{
  // Relation, element to vertex 1->3 or 1->4
  ...
  for(unsigned int i=0 ; i < element_element_vec.size() ; i++)
  {
    ...
    element_element_vec[i] = ElementToElementRelation::INVALID_INDEX;
  }
  ...
  //Relation, Element to element. 1->3 for triangles, 1->4 for tetrahedrons
  for(int i=0 ; i<element_set.size() ; i++)
  {
...
template <unsigned int S, unsigned int D, typename P>
typename IAMesh<S,D,P>::IndexListType IAMesh<S,D,P>::
getVerticesInElement(IndexType element_idx) const
{
  IndexListType ret;
  ...
  typename ElementToVertexRelation::RelationSet rvec = ev_rel[element_idx];
...
template <unsigned int S, unsigned int D, typename P>
typename IAMesh<S,D,P>::IndexListType IAMesh<S,D,P>::
getElementsWithVertex(  IndexType vertex_idx) const
{
  IndexListType ret;
  ...
  while( element_traverse_queue.size() > 0)
  {
    ...
    typename ElementToElementRelation::RelationSet surrounded_elements =
      ee_rel[element_i];
    ...
    for(int i=0 ; i<surrounded_elements.size() ; i++)
    {
      ...
      if (ele_i == ElementToElementRelation::INVALID_INDEX
          || is_subset(ele_i, ret))
        continue;
...
template <unsigned int S, unsigned int D, typename P>
typename IAMesh<S,D,P>::IndexListType IAMesh<S,D,P>::
getElementFace( IndexType element_idx, IndexType face_idx) const
{
  IndexListType ret;
  ...
  {
    SLIC_WARNING(
      "Attempting to retrieve data with an invalid element: "
      << element_idx);
  ...
  SLIC_ASSERT_MSG(
    0 <= face_idx && face_idx < VERTS_PER_ELEM,
...
template <unsigned int S, unsigned int D, typename P>
typename IAMesh<S,D,P>::IndexListType IAMesh<S,D,P>::
getElementNeighbors( IndexType element_idx) const
{
  IndexListType ret;
  ...
  typename ElementToVertexRelation::RelationSet rvec = ee_rel[element_idx];
  ret.resize(rvec.size());
  for(int i=0 ; i<rvec.size() ; i++)
  {
    ret[i] = rvec[i];
...
template <unsigned int S, unsigned int D, typename P>
const typename IAMesh<S,D,P>::Point& IAMesh<S,D,P>::
getVertexPoint( IndexType vertex_idx) const
{
...
template <unsigned int S, unsigned int D, typename P>
void IAMesh<S,D,P>::removeVertex(IndexType vertex_idx)
{
  ...
  std::vector<IndexType> attached_elements =
    getElementsWithVertex( vertex_idx );
  ...
  //vcoord_map.remove(vertex_idx); //once set entry is removed, its corrsponding
  // map entry is assumed to be invalid
}


template <unsigned int S, unsigned int D, typename P>
void IAMesh<S,D,P>::removeElement(IndexType element_idx)
{
  if(!element_set.isValidEntry(element_idx))
  {
  ...
  for(int i=0 ; i<ev_rel[ element_idx ].size() ; i++)
  {
    ...
    { //the element being removed is the last element using this vertex
      ve_rel.modify(vertex_i, 0, VertexToOneElementRelation::INVALID_INDEX);
    }
    else
    {
  ...
  for(int zi=0 ; zi< ee_rel[element_idx].size() ; zi++)
  {
    ...
    if(nbr_element_idx < 0)
      continue;
    for(int i=0 ; i<ee_rel[nbr_element_idx].size() ; i++)
    {
      if( ee_rel[nbr_element_idx][i] == element_idx )
      {
        ee_rel.modify(nbr_element_idx,i,ElementToVertexRelation::INVALID_INDEX);
        break;
      }
    }
  }
...
template <unsigned int S, unsigned int D, typename P>
typename IAMesh<S,D,P>::IndexType IAMesh<S,D,P>::addVertex(const Point& p)
{
  IndexType vertex_idx = vertex_set.insert();
  vcoord_map.insert(vertex_idx, p);
  ve_rel.insert(vertex_idx, VertexToOneElementRelation::INVALID_INDEX);

  return vertex_idx;
}

template <unsigned int S, unsigned int D, typename P>
typename IAMesh<S,D,P>::IndexType IAMesh<S,D,P>::
addElement(IndexType n0, IndexType n1, IndexType n2)
{
  SLIC_ASSERT(VERTS_PER_ELEM==3); //Make sure the right kind of element is added
  IndexType nlist[] = {n0,n1,n2};
  return addElement( nlist );
}

template <unsigned int S, unsigned int D, typename P>
typename IAMesh<S,D,P>::IndexType IAMesh<S,D,P>::
addElement(IndexType n0, IndexType n1, IndexType n2, IndexType n3)
{
  SLIC_ASSERT(VERTS_PER_ELEM==4); //Make sure the right kind of element is added
...
template <unsigned int S, unsigned int D, typename P>
typename IAMesh<S,D,P>::IndexType IAMesh<S,D,P>::
addElement( const IndexType* nlist)
{
  ...
  for(int i=0 ; i<VERTS_PER_ELEM ; i++)
    ev_rel.insert(element_idx, nlist[i]);

  //make sure the space is allocated in ee_rel
  ee_rel.insert(element_idx, ElementToElementRelation::INVALID_INDEX);
  ...
  for( std::set<IndexType>::iterator it = elem_list.begin() ;
       it != elem_list.end() ; it++)
  {
...
template <unsigned int S, unsigned int D, typename P>
void IAMesh<S,D,P>::
fixVertexNeighborhood(IndexType vertex_idx,
                      const std::vector<IndexType> & new_elements)
{
  typedef std::pair<IndexType, IndexType>             IndexPairType;
  typedef std::map<IndexListType, IndexPairType>      FaceVertMapType;
  typedef std::pair<IndexListType, IndexPairType>     FaceVertPairType;
  ...
  for(unsigned int i=0 ; i<new_elements.size() ; i++)
  {
    ...
    for(int face_i=0 ; face_i < VERTS_PER_ELEM ; face_i++)
    {
      ...
      if( !is_subset(vertex_idx, fv_list ))
        continue;
      ...
      std::pair< FaceVertMapType::iterator, bool > ret =
        vert_map.insert( FaceVertPairType( fv_list,
                                           IndexPairType(el, face_i)) );
...
void IAMesh<S,D,P>::compact(){
  ...
  typename ElementToVertexRelation::RelationVec& ev_rel_data = ev_rel.data();
  for(int i=0 ; i < (int)ev_rel.size() ; i++ )
  {
    ...
    for(int j=0 ; j<(int)ev_rel[i].size() ; j++ )
    {
      ...
      if(val != ElementToVertexRelation::INVALID_INDEX )
      {
  ...
  typename ElementToVertexRelation::RelationVec & ve_rel_data = ve_rel.data();
  for(int i=0 ; i < (int)ve_rel.size() ; i++ )
  {
    ...
    if(val != VertexToOneElementRelation::INVALID_INDEX )
    {
  ...
  typename ElementToVertexRelation::RelationVec & ee_rel_data = ee_rel.data();
  for(int i=0 ; i < (int)ee_rel.size() ; i++ )
  {
    ...
    for(int j=0 ; j<(int)ee_rel[i].size() ; j++ )
    {
      ...
      if(val != ElementToElementRelation::INVALID_INDEX )
      {
        val = element_set_map[val];
      }
...
template <unsigned int S, unsigned int D, typename P>
bool IAMesh<S,D,P>::isEmpty() const {
  return vertex_set.size() == 0;
}

template <unsigned int S, unsigned int D, typename P>
bool IAMesh<S,D,P>::isManifold(bool verboseOutput) const
{
  /* TODO check that...
...
template <unsigned int S, unsigned int D, typename P>
bool IAMesh<S,D,P>::isValid(bool verboseOutput) const
{
  std::stringstream errSstr;
  ...
  if( vertex_set.size() != ve_rel.size() ||
      vertex_set.size() != vcoord_map.size() )
  {
    if(verboseOutput)
    {
  ...
  if( element_set.size() != ev_rel.size() ||
      element_set.size() != ee_rel.size()  )
  {
    if(verboseOutput)
    {
  ...
  for(IndexType pos = 0 ; pos < element_set.size() ; ++pos)
  {
    if (element_set.isValidEntry(pos))
    {
      for(IndexType rpos = 0 ; rpos < ev_rel[pos].size() ; rpos++ )
      {
        ...
        if(ev_rel[pos][rpos] == ElementToVertexRelation::INVALID_INDEX)
        {
          if(verboseOutput)
          {
            errSstr
