...
template <typename KVPair,
          ...
          typename StoragePair = TypeErasedStorage<KVPair>>
...
#endif
  IndexType index =
    LookupPolicy {}.nextValidIndex(metadata, LookupPolicy::NO_MATCH);
                        ...
                        axom::ArrayView<StoragePair> to_buckets)
{
  ...
  {
    axom::copy(to_buckets.data(),
               from_buckets.data(),
               sizeof(StoragePair) * from_buckets.size());
    return;
  }
  ...
  if(to_space == MemorySpace::Device)
  {
    ...
    // our desired buffer.
    axom::copy(to_buckets.data(),
               to_buckets_stage.data(),
               sizeof(StoragePair) * to_buckets.size());
  }
#endif
}
