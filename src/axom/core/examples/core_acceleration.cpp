...
void demoMemoryManageBasic()
{
  ...
  for(int i = 0; i < len; i++){
    ...
    std::cout << "Matches old value? " <<  std::boolalpha << (dynamic_memory_array[i] == dyn_array_dst[i]) << std::endl;
  }
...
void demoAxomExecution()
{
  ...
  axom::for_all< axom::SEQ_EXEC >(0, 1000, AXOM_LAMBDA( axom::IndexType i ) {
    C[i] = A[i] + B[i];
  });
