...
// protected method
View*
MFEMSidreDataCollection::alloc_view(Group* grp,
...
// protected method
Group*
MFEMSidreDataCollection::alloc_group(Group* grp,
...
// protected method
std::string
MFEMSidreDataCollection::get_file_path(const std::string &filename) const
{
...
View*
MFEMSidreDataCollection::AllocNamedBuffer(const std::string& buffer_name,
                                          IndexType sz,
                                          TypeID type)
{
  ...
  else
  {
    ...
    SLIC_ASSERT_MSG(v->getTypeID() == type,
                                            ...
                                            IndexType offset)
{
  ...
  SLIC_ASSERT_MSG(
    mesh != nullptr,
    "Need to set mesh before registering attributes in MFEMSidreDataCollection.");
...
void MFEMSidreDataCollection::RegisterAttributeField(
  ...
  bool is_bdry)
{
  ...
  mfem::Array<int>* attr = new mfem::Array<int>(
    a->getData<int*>(), a->getNumElements());
...
void MFEMSidreDataCollection::DeregisterAttributeField(
  const std::string& attr_name)
{
...
{
  sidre::Group* fields_grp = m_bp_index_grp->getGroup("fields");
  SLIC_WARNING_IF(!fields_grp->hasGroup(
                    attr_name),
                  "No attribute exists in blueprint index with name " <<
      attr_name);
...
// private method
std::string MFEMSidreDataCollection::getElementName(
  mfem::Element::Type elementEnum)
{
