...
public:
  ...
  template <typename ExecSpace>
  void runShapeQuery(const klee::Shape& shape)
  {
    ...
    for(int i = 0; i < NE; i++)
    {
      ...
      // Set each shape volume fraction to 0
      double vf = 1.0;
      (*volFrac)(i) = vf;
      ...
      {
        m_hexes[i].addVertex(
          {vertCoords[j][0], vertCoords[j][1], vertCoords[j][2]});
...
public:
  // Empty implementation (use templated version)
