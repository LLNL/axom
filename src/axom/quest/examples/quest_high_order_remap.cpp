...
public:
  ...
  void setMesh(mfem::Mesh* mesh)
  {
    ...
    bool isHighOrder = (m_mesh->GetNodalFESpace() != nullptr)
                       && (m_mesh->GetNE() > 0);
...
private:
  ...
  {
    return Point::make_point(
      (*nodes)(fes->DofToVDof(idx, 0)),
...
public:
  ...
  mfem::GridFunction * project_to_pos_basis(const mfem::GridFunction *gf, bool &is_new)
  {
    ...
    if (nodal_fe_coll == nullptr) { std::cerr << "project_to_pos_basis(): nodal_fe_coll is NULL!" << std::endl; }
      ...
      auto* pos_fe_coll = new mfem::H1_FECollection(5, dim,
  ...
  void setupMeshes(int res1, int res2, int order)
  {
    ...
    // create the source mesh
    {
      ...
      auto* fec = new mfem::H1_FECollection(src_ord, dim,
                                            mfem::BasisType::Positive);
      fecMap.Register("src_fec", fec, true);
  ...
  CandidateList getSourceCandidates(int targetElemID) const
  {
    ...
    for (auto idx = candidateBits.find_first() ;
         idx != BitsetType::npos ;
