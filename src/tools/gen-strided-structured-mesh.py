...
# gen-strided-structured-mesh.py
# Write a simple strided-structured blueprint mesh for testing.
...
def scale_structured_domain(n, startCoord, endCoord):
  ...
  for d in range(ndim):
    ...
    if ndim == 2:
      realCoords = coords[npnl:, npnl:] if npnr == 0 else coords[npnl:-npnr, npnl:-npnr]
    else:
      realCoords = coords[npnl:, npnl:, npnl:] if npnr == 0 else coords[npnl:-npnr, npnl:-npnr, npnl:-npnr]
...
#print(f'meshLower={meshLower} meshUpper={meshUpper}')
