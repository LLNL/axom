...
template <class Char> class formatbuf : public std::basic_streambuf<Char> {
 ...
 protected:
  ...
  int_type overflow(int_type ch = traits_type::eof()) AXOM_FMT_OVERRIDE {
    if (!traits_type::eq_int_type(ch, traits_type::eof()))
      buffer_.push_back(static_cast<Char>(ch));
    return ch;
  }

  std::streamsize xsputn(const Char* s, std::streamsize count) AXOM_FMT_OVERRIDE {
...
struct converter {
  template <typename T, AXOM_FMT_ENABLE_IF(is_integral<T>::value)> converter(T);
};
...
struct fallback_formatter<T, Char, enable_if_t<is_streamable<T, Char>::value>>
  ...
  template <typename ParseCtx,
            AXOM_FMT_ENABLE_IF(std::is_same<
