...
TEST(C_sidre_opaque,basic_inout)
{
  ...
  SIDRE_dataview * ext_view = SIDRE_group_create_view_external(problem_gp,
                                                                   "ext", ext);

  bool test_external = SIDRE_dataview_is_external(ext_view);
  EXPECT_EQ(test_external, true);

  bool test_applied = SIDRE_dataview_is_applied(ext_view);
  EXPECT_EQ(test_applied, false);

  bool test_opaque = SIDRE_dataview_is_opaque(ext_view);
  EXPECT_EQ(test_opaque, true);

  AA_extent * test_extent = (AA_extent *) SIDRE_dataview_get_void_ptr(ext_view);
  ...
  SIDRE_dataview * ext2_view = SIDRE_group_create_view_empty(problem_gp,
                                                                 "ext2");
...
//  SIDRE_datastore_print(ds);
  ...
  for (int idom = 0 ; idom < 2 ; ++idom)
  {
    ...
    SIDRE_dataview * dom_zone_data_view = SIDRE_group_get_view_from_name(
      dom_gp, "zone_data");
    ...
    SIDRE_dataview * dom_node_data_view = SIDRE_group_get_view_from_name(
      dom_gp, "node_data");
