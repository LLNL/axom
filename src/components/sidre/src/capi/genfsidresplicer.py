...
def create_allocatable_view(d):
    ...
    # f_type   - fortran type
    # shape     - :,:, to match nd
    callname = 'SIDRE_create_allocatable_view_{typename}_{nd}'.format(**d)
    return """
! Generated by genfsidresplicer.py
function datagroup_create_allocatable_view_{typename}_{nd}(group, name, value) result(rv)
    ...
    integer(C_INT), parameter :: itype = {sidre_type}
...
subroutine dataview_get_data_{typename}_{nd}{suffix}(view, value)
    ...
    call c_f_pointer(cptr, value, [ nelems ])
        ...
        raise RuntimeError("rank too large in print_get_data")
...
def gen_fortran():
    ...
    print_lines(print, iface_sidre_c_loc_allocatable)
...
def iface_sidre_c_loc_allocatable(d):
    return """
   subroutine sidre_c_loc_allocatable_{typename}_{nd}(variable, addr)
     use iso_c_binding
     {f_type}, allocatable, intent(IN) :: variable{shape}
     type(C_PTR), intent(OUT) :: addr
   end subroutine sidre_c_loc_allocatable_{typename}_{nd}""".format(**d)

def print_sidre_c_loc_allocatable(d):
    """Write C++ routine to accept Fortran allocatable.
    """
    name = 'sidre_c_loc_allocatable_{typename}_{nd}'.format(**d)
    return """
...
def print_sidre_size_allocatable(d):
    """Write Fortran routine to return size of allocatable.
    """
    if d['rank'] == 0:
        return """
function sidre_size_allocatable_{typename}_{nd}(array) result(rv)
    use iso_c_binding
    implicit none
    {f_type}, allocatable, intent(IN) :: array{shape}
    integer(C_SIZE_T) :: rv
    rv = 1
end function sidre_size_allocatable_{typename}_{nd}""".format(**d)
    else:
        return """
function sidre_size_allocatable_{typename}_{nd}(array) result(rv)
    use iso_c_binding
...
def print_sidre_size_allocatable_header(d):
    """Write C++ declarations for Fortran routines.
    """
    name = 'size_allocatable_{typename}_{nd}'.format(**d)
    return """#define {upper} FC_GLOBAL(sidre_{lower},SIDRE_{upper})
...
def print_sidre_address_allocatable(d):
    """Write Fortran routine to return address of allocatable.
    Use C_LOC and add TARGET attribute
    """
    return """
subroutine sidre_address_allocatable_{typename}_{nd}(array, addr)
    use iso_c_binding
    implicit none
    {f_type}, allocatable, intent(IN), target :: array{shape}
    type(C_PTR), intent(OUT) :: addr
    addr = c_loc(array)
end subroutine sidre_address_allocatable_{typename}_{nd}""".format(**d)

def print_sidre_address_allocatable_header(d):
...
def print_sidre_allocate_allocatable(d):
    ...
    return """
subroutine sidre_allocate_allocatable_{typename}_{nd}(array, nitems)
    use iso_c_binding
    implicit none
    {f_type}, allocatable, intent(OUT), target :: array{shape}
    integer(C_INT), intent(IN) :: nitems
    allocate(array{size})
end subroutine sidre_allocate_allocatable_{typename}_{nd}""".format(size=size, **d)

def print_sidre_allocate_allocatable_header(d):
...
def print_sidre_deallocate_allocatable(d):
    """Write Fortran routine to deallocate an allocatable.
    """
    return """
subroutine sidre_deallocate_allocatable_{typename}_{nd}(array)
    use iso_c_binding
    implicit none
    {f_type}, allocatable, intent(INOUT), target :: array{shape}
    deallocate(array)
end subroutine sidre_deallocate_allocatable_{typename}_{nd}""".format(**d)

def print_sidre_deallocate_allocatable_header(d):
...
def print_sidre_reallocate_allocatable(d):
    """Write Fortran routine to allocate an allocatable.
    """
    if d['rank'] == 0:
        size = '';
    ...
    return """
subroutine sidre_reallocate_allocatable_{typename}_{nd}(array, nitems)
    use iso_c_binding
    implicit none
    {f_type}, allocatable, intent(INOUT), target :: array{shape}
    ...
    allocate(array{size})
! XXX move_alloc ...
end subroutine sidre_reallocate_allocatable_{typename}_{nd}""".format(size=size, **d)

def print_sidre_reallocate_allocatable_header(d):
