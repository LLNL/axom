...
def type_bound_procedure_part(d):
    return 'procedure :: {stem}_{typename}_{nd} => dataview_get_value_{typename}_{nd}'.format(**d)
...
def foreach_value(lines, fcn, stem, **kwargs):
...
#    print()
    ...
    # DataView
    lines = []
    foreach_value(lines, type_bound_procedure_part, 'get_value')
    generics = []
    foreach_value(generics, type_bound_procedure_generic, 'get_value')
    ...
    lines = []
    foreach_value(lines, print_get_value, 'get_value')
    for line in lines:
        print(line)
