...
def XXXcreate_allocatable_view(d):
    # typename - part of function name
    # nd       - number of dimensions
    # f_type   - fortran type
    # shape     - :,:, to match nd
...
def XXiface_sidre_c_loc_allocatable(d):
    return """
...
def XXprint_sidre_c_loc_allocatable(d):
...
def XXprint_sidre_size_allocatable(d):
...
def XXprint_sidre_size_allocatable_header(d):
...
def XXprint_sidre_address_allocatable(d):
...
def XXprint_sidre_allocate_allocatable(d):
...
def XXprint_sidre_deallocate_allocatable(d):
...
def XXprint_sidre_deallocate_allocatable_header(d):
    name = 'deallocate_allocatable_{typename}_{nd}'.format(**d)
    return """#define {upper} FC_GLOBAL(sidre_{lower},SIDRE_{upper})
void {upper}(void * array);
""".format(lower=name.lower(), upper=name.upper())

def XXDeallocateAllocatable(printer):
    calls = [
...
def XXprint_sidre_reallocate_allocatable(d):
    """Write Fortran routine to allocate an allocatable.
    """
    if d['rank'] == 0:
        size = '';
...
def XXprint_sidre_reallocate_allocatable_header(d):
