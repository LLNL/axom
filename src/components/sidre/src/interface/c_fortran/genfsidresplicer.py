...
subroutine group_get_scalar_{typename}(grp, name, value)
    ...
    lname = len_trim(name)
    tview = c_group_get_view_from_name_bufferify(grp%voidptr, name, lname)
    value = c_view_get_data_{typename}(tview)
...
subroutine group_set_scalar_{typename}(grp, name, value)
    ...
    lname = len_trim(name)
    tview = c_group_get_view_from_name_bufferify(grp%voidptr, name, lname)
    call c_view_set_scalar_{typename}(tview, value)
...
def group_set_array_data_ptr(d):
    """
    call tview%set_external_data_ptr
...
!    {extents_asgn}
    tview = c_group_get_view_from_name_bufferify(grp%voidptr, name, lname)
    if (c_associated(tview)) then
#ifdef USE_C_LOC_WITH_ASSUMED_SHAPE
        addr = c_loc(value)
#else
        call SHROUD_C_LOC(value{lower_bound}, addr)
#endif
        call c_view_set_external_data_ptr_only(tview, addr)
!        call c_view_apply_type_shape(rv%voidptr, type, {rank}, extents)
    endif
...
def view_set_array_data_ptr(d):
    ...
    return """
! Generated by genfsidresplicer.py
subroutine view_set_array_data_ptr_{typename}{nd}(tview, value)
    use iso_c_binding
    implicit none

    class(view), intent(IN) :: tview
    {f_type}, target, intent(IN) :: value{shape}
!    integer(SIDRE_LENGTH) :: {extents_decl}
!    integer(C_INT), parameter :: type = {sidre_type}
    type(C_PTR) addr
...
!    {extents_asgn}
    ...
    call SHROUD_C_LOC(value{lower_bound}, addr)
#endif
    call c_view_set_external_data_ptr_only(tview%voidptr, addr)
!    call c_view_apply_type_shape(rv%voidptr, type, {rank}, extents)
...
def print_get_data(d):
    ...
    if d['rank'] == 0:
        return """
! Generated by genfsidresplicer.py
subroutine view_get_data_{typename}{nd}{suffix}(tview, value)
    use iso_c_binding
    implicit none
    class(view), intent(IN) :: tview
    ...
    cptr = tview%get_void_ptr()
    if (c_associated(cptr)) then
      offset = tview%get_offset()
      if (offset > 0) then
...
subroutine view_get_data_{typename}{nd}{suffix}(tview, value)
    ...
    if (c_associated(cptr)) then
      ...
      rank = tview%get_shape({rank}, extents)
...
subroutine group_get_string(grp, name, value)
    ...
    lname = len_trim(name)
    tview = c_group_get_view_from_name_bufferify(grp%voidptr, name, lname)
    call c_view_get_string_bufferify(tview, value, len(value, kind=C_INT))
...
subroutine group_set_string(grp, name, value)
    ...
    lname = len_trim(name)
    tview = c_group_get_view_from_name_bufferify(grp%voidptr, name, lname)
    call c_view_set_string_bufferify(tview, value, len_trim(value, kind=C_INT))
