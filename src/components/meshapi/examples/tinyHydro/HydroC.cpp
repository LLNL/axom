    ...
    , Cq(1.0), Cl(1.0), cfl(0.7)
{
   ...
   for (int n = 1; n < mesh.numNodes(); n++)  // compute bounding box of domain
   {
      ...
      if (pos.x > bbUpper.x) bbUpper.x = pos.x;
      if (pos.y < bbLower.y) bbLower.y = pos.y;
   ...
   {
      const VectorXY & pos = mesh.nodePos[n];
      if (fuzzyEqual(pos.y, bbLower.y)) bcNodeLists[0].push_back(n);
      if (fuzzyEqual(pos.x, bbUpper.x)) bcNodeLists[1].push_back(n);
      if (fuzzyEqual(pos.y, bbUpper.y)) bcNodeLists[2].push_back(n);
