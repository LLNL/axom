...
#include "mint/database.hpp"            // for database::get_view
...
public:
  ...
  /*!
   * \brief Constructs a Vector instance with the given number of tuples
   *  and capacity.
   ...
   */
  Array( localIndex num_tuples, localIndex capacity, 
         localIndex num_components );

#ifdef MINT_USE_SIDRE
  /*!
   * \brief Constructor for use with a sidre::View that already has data.
   * \param [in] path the path to the sidre::View that holds this Vector's data.
   * \param [in] num_tuples the number of tuples accounted for in the vector.
   */
  Array( const std::string & path, localIndex num_tuples );
   ...
   * \param [in] num_components the number of values per tuple.
   */
  Array( const std::string & path, localIndex num_tuples, localIndex capacity,
         localIndex num_components );
#endif
   ...
   */
  inline T & operator()( localIndex pos, localIndex component=1 ) 
   ...
   * \return a reference to the given component of the specified tuple.
   */
  inline T & operator()( localIndex pos, localIndex component=1 ) const
  { return m_data[ pos * m_num_components + component ]; }
   ...
   * \note Reallocation is done if the new size will exceed the capacity.
   */
  inline void setNumTuples( localIndex num_tuples );
...
private:
   ...
   */
  inline localIndex getViewShape( bool dim ) const;
...
//------------------------------------------------------------------------------
template< typename T >
Array< T >::Array( localIndex num_tuples, localIndex capacity, 
                   localIndex num_components ) :
#ifdef MINT_USE_SIDRE
  m_view( AXOM_NULLPTR ),
#endif
...
//------------------------------------------------------------------------------
template< typename T >
Array< T >::Array( const std::string & path, localIndex num_tuples ) :
  ...
  m_resize_ratio( DEFAULT_RESIZE_RATIO )
{
  SLIC_ERROR_IF( m_view == AXOM_NULLPTR, "View not found: " << path );
...
//------------------------------------------------------------------------------
template< typename T >
Array< T >::Array( const std::string & path, localIndex num_tuples, localIndex capacity,
                   localIndex num_components ) :
  m_view( database::get_view( path ) ),
  m_data( AXOM_NULLPTR )
  m_num_tuples( num_tuples ),
...
{
  new_size = m_num_tuples + n;
  if ( new_size > m_capacity )
  { dynamicRealloc( new_size ); }

  T* cur_end = m_data + m_num_tuples * m_num_components;
  std::memcpy( cur_end, tuples, n * * m_num_tuples * sizeof(T) );
  m_num_tuples = new_size;
}
...
//------------------------------------------------------------------------------
template< typename T >
inline void Array< T >::setNumTuples( localIndex num_tuples )
{
...
//------------------------------------------------------------------------------
template< typename T >
inline Array< T >::T* reserveForInsert( localIndex n, localIndex pos )
{
...
//------------------------------------------------------------------------------
template< typename T >
inline localIndex Array< T >::getViewShape( bool dim ) const
{
