...
/*************** INTERSECT TESTS MOVED OVER TO primal_intersect ****************
//------------------------------------------------------------------------------
TEST( primal_OBBox, obb_intersects )
{
  static const int DIM = 3;
  typedef double CoordType;
  typedef primal::Point< CoordType, DIM > QPoint;
  ...
  EXPECT_TRUE(obbox1.intersects(obbox2));
  QOBBox obbox3(obbox1);
  obbox1.shift(e/2.);

  EXPECT_TRUE(obbox1.intersects(obbox3));
  EXPECT_TRUE(obbox3.intersects(obbox1));

  obbox1.shift(e/2.);
  EXPECT_TRUE(obbox1.intersects(obbox3));

  obbox1.shift(e/2.);
  EXPECT_FALSE(obbox1.intersects(obbox3));
  EXPECT_FALSE(obbox3.intersects(obbox1));

  EXPECT_TRUE(obbox1.intersects(obbox1));
  EXPECT_TRUE(obbox2.intersects(obbox2));
  EXPECT_TRUE(obbox3.intersects(obbox3));
}******************************************************************************/
...
//------------------------------------------------------------------------------
/*TEST( primal_OBBox, obb_test_closest_point )
{
  static const int DIM = 3;
  typedef double CoordType;
  typedef primal::Point< CoordType, DIM > QPoint;
  ...
  EXPECT_TRUE(obbox1.closestPoint(pt1) == pt1);
  EXPECT_TRUE(obbox1.closestPoint(pt3) == pt2);

  QPoint pt4;
  QPoint pt5;
  pt4.array()[2] = 2.;
  pt5.array()[2] = 1.;
  EXPECT_TRUE(obbox1.closestPoint(pt4) == pt5);
