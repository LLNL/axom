...
TEST( primal_clipping, experimentalData)
{
  ...
  TriangleType tri(
        PointType::make_point(0.405431,3.91921,3.07821),
  ...
  BoundingBoxType box12(
      PointType::make_point(0.937594,4.06291,2.50025),
  ...
  for(int i=0; i< poly.numVertices(); ++i)
  {
    ...
    SLIC_INFO("Testing clipped polygon point " << poly[i]
             << "-- w/ barycentric coords " << bary
  ...
  // Check that the polygon centroid is on the triangle
  {
    ...
    SLIC_INFO("Testing clipped polygon centroid " << centroid
             << "-- w/ barycentric coords " << bary
             << "\n\t-- reconstructed point is: " << reconstructed
             << "...\n");
