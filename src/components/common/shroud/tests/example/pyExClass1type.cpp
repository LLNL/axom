// pyExClass1type.cpp
// This is generated code, do not edit
// blah blah
// yada yada
//
#include "pyUserLibrarymodule.hpp"
// splicer begin class.ExClass1.C_definition
// splicer end class.ExClass1.C_definition
// splicer begin class.ExClass1.extra_methods
// splicer end class.ExClass1.extra_methods
// dealloc
static void
PP_ExClass1_dealloc (PP_ExClass1 *self)
{
// splicer begin class.ExClass1.type.dealloc
    PyErr_SetString(PyExc_NotImplementedError, "dealloc");
    return ;
// splicer end class.ExClass1.type.dealloc
}
// print
static int
PP_ExClass1_print (PP_ExClass1 *self, FILE *fp, int flags)
{
// splicer begin class.ExClass1.type.print
    PyErr_SetString(PyExc_NotImplementedError, "print");
    return -1;
// splicer end class.ExClass1.type.print
}
// compare
static int
PP_ExClass1_compare (PP_ExClass1 *self, PyObject *)
{
// splicer begin class.ExClass1.type.compare
    PyErr_SetString(PyExc_NotImplementedError, "compare");
    return -1;
// splicer end class.ExClass1.type.compare
}
// getattr
static PyObject *
PP_ExClass1_getattr (PP_ExClass1 *self, char *name)
{
// splicer begin class.ExClass1.type.getattr
    PyErr_SetString(PyExc_NotImplementedError, "getattr");
    return NULL;
// splicer end class.ExClass1.type.getattr
}
// setattr
static int
PP_ExClass1_setattr (PP_ExClass1 *self, char *name, PyObject *value)
{
// splicer begin class.ExClass1.type.setattr
    PyErr_SetString(PyExc_NotImplementedError, "setattr");
    return -1;
// splicer end class.ExClass1.type.setattr
}
// getattro
static PyObject *
PP_ExClass1_getattro (PP_ExClass1 *self, PyObject *name)
{
// splicer begin class.ExClass1.type.getattro
    PyErr_SetString(PyExc_NotImplementedError, "getattro");
    return NULL;
// splicer end class.ExClass1.type.getattro
}
// setattro
static int
PP_ExClass1_setattro (PP_ExClass1 *self, PyObject *name, PyObject *value)
{
// splicer begin class.ExClass1.type.setattro
    PyErr_SetString(PyExc_NotImplementedError, "setattro");
    return -1;
// splicer end class.ExClass1.type.setattro
}
// repr
static PyObject *
PP_ExClass1_repr (PP_ExClass1 *self)
{
// splicer begin class.ExClass1.type.repr
    PyErr_SetString(PyExc_NotImplementedError, "repr");
    return NULL;
// splicer end class.ExClass1.type.repr
}
// hash
static long
PP_ExClass1_hash (PP_ExClass1 *self)
{
// splicer begin class.ExClass1.type.hash
    PyErr_SetString(PyExc_NotImplementedError, "hash");
    return -1;
// splicer end class.ExClass1.type.hash
}
// call
static PyObject *
PP_ExClass1_call (PP_ExClass1 *self, PyObject *args, PyObject *kwds)
{
// splicer begin class.ExClass1.type.call
    PyErr_SetString(PyExc_NotImplementedError, "call");
    return NULL;
// splicer end class.ExClass1.type.call
}
// str
static PyObject *
PP_ExClass1_str (PP_ExClass1 *self)
{
// splicer begin class.ExClass1.type.str
    PyErr_SetString(PyExc_NotImplementedError, "str");
    return NULL;
// splicer end class.ExClass1.type.str
}
// init
static int
PP_ExClass1_init (PP_ExClass1 *self, PyObject *args, PyObject *kwds)
{
// splicer begin class.ExClass1.type.init
    PyErr_SetString(PyExc_NotImplementedError, "init");
    return -1;
// splicer end class.ExClass1.type.init
}
// alloc
static PyObject *
PP_ExClass1_alloc (PyTypeObject *type, Py_ssize_t nitems)
{
// splicer begin class.ExClass1.type.alloc
    PyErr_SetString(PyExc_NotImplementedError, "alloc");
    return NULL;
// splicer end class.ExClass1.type.alloc
}
// new
static PyObject *
PP_ExClass1_new (PyTypeObject *type, PyObject *args, PyObject *kwds)
{
// splicer begin class.ExClass1.type.new
    PyErr_SetString(PyExc_NotImplementedError, "new");
    return NULL;
// splicer end class.ExClass1.type.new
}
// free
static void
PP_ExClass1_free (void *op)
{
// splicer begin class.ExClass1.type.free
    PyErr_SetString(PyExc_NotImplementedError, "free");
    return ;
// splicer end class.ExClass1.type.free
}
// del
static void
PP_ExClass1_del (PP_ExClass1 *self)
{
// splicer begin class.ExClass1.type.del
    PyErr_SetString(PyExc_NotImplementedError, "del");
    return ;
// splicer end class.ExClass1.type.del
}

static char PP_exclass1_new__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_new(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.new
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.new
}

static char PP_exclass1_delete__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_delete(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.delete
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.delete
}

static char PP_exclass1_increment_count__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_increment_count(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.increment_count
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.increment_count
}

static char PP_exclass1_get_name__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_get_name(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.get_name
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.get_name
}

static char PP_exclass1_get_name_length__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_get_name_length(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.get_name_length
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.get_name_length
}

static char PP_exclass1_get_root__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_get_root(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.get_root
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.get_root
}

static char PP_exclass1_get_value_from_int__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_get_value_from_int(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.get_value_from_int
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.get_value_from_int
}

static char PP_exclass1_get_value_1__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_get_value_1(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.get_value_1
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.get_value_1
}

static char PP_exclass1_get_addr__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_get_addr(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.get_addr
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.get_addr
}

static char PP_exclass1_has_addr__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_has_addr(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.has_addr
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.has_addr
}

static char PP_exclass1_splicer_special__doc__[] =
"documentation"
;

static PyObject *
PP_exclass1_splicer_special(
  PyObject *self,    /* not used */
  PyObject *args,
  PyObject *kwds)
{
// splicer begin class.ExClass1.method.splicer_special
PyErr_SetString(PyExc_NotImplementedError, "XXX");
return NULL;
// splicer end class.ExClass1.method.splicer_special
}
static PyMethodDef PB_methods[] = {
{"new", (PyCFunction)PP_exclass1_new, METH_VARARGS|METH_KEYWORDS, PP_exclass1_new__doc__},
{"delete", (PyCFunction)PP_exclass1_delete, METH_VARARGS|METH_KEYWORDS, PP_exclass1_delete__doc__},
{"incrementCount", (PyCFunction)PP_exclass1_increment_count, METH_VARARGS|METH_KEYWORDS, PP_exclass1_increment_count__doc__},
{"getName", (PyCFunction)PP_exclass1_get_name, METH_VARARGS|METH_KEYWORDS, PP_exclass1_get_name__doc__},
{"get_name_length", (PyCFunction)PP_exclass1_get_name_length, METH_VARARGS|METH_KEYWORDS, PP_exclass1_get_name_length__doc__},
{"getRoot", (PyCFunction)PP_exclass1_get_root, METH_VARARGS|METH_KEYWORDS, PP_exclass1_get_root__doc__},
{"getValue_from_int", (PyCFunction)PP_exclass1_get_value_from_int, METH_VARARGS|METH_KEYWORDS, PP_exclass1_get_value_from_int__doc__},
{"getValue_1", (PyCFunction)PP_exclass1_get_value_1, METH_VARARGS|METH_KEYWORDS, PP_exclass1_get_value_1__doc__},
{"getAddr", (PyCFunction)PP_exclass1_get_addr, METH_VARARGS|METH_KEYWORDS, PP_exclass1_get_addr__doc__},
{"hasAddr", (PyCFunction)PP_exclass1_has_addr, METH_VARARGS|METH_KEYWORDS, PP_exclass1_has_addr__doc__},
{"SplicerSpecial", (PyCFunction)PP_exclass1_splicer_special, METH_VARARGS|METH_KEYWORDS, PP_exclass1_splicer_special__doc__},
{NULL,   (PyCFunction)NULL, 0, NULL}            /* sentinel */
};

static char ExClass1__doc__[] =
"virtual class"
;

/* static */
PyTypeObject PP_ExClass1_Type = {
        PyVarObject_HEAD_INIT(NULL, 0)
        "userlibrary.ExClass1",                       /* tp_name */
        sizeof(PP_ExClass1),         /* tp_basicsize */
        0,                              /* tp_itemsize */
        /* Methods to implement standard operations */
        (destructor)PP_ExClass1_dealloc,                 /* tp_dealloc */
        (printfunc)PP_ExClass1_print,                   /* tp_print */
        (getattrfunc)PP_ExClass1_getattr,                 /* tp_getattr */
        (setattrfunc)PP_ExClass1_setattr,                 /* tp_setattr */
#ifdef IS_PY3K
        0,                               /* tp_reserved */
#else
        (cmpfunc)PP_ExClass1_compare,                     /* tp_compare */
#endif
        (reprfunc)PP_ExClass1_repr,                    /* tp_repr */
        /* Method suites for standard classes */
        0,                              /* tp_as_number */
        0,                              /* tp_as_sequence */
        0,                              /* tp_as_mapping */
        /* More standard operations (here for binary compatibility) */
        (hashfunc)PP_ExClass1_hash,                    /* tp_hash */
        (ternaryfunc)PP_ExClass1_call,                 /* tp_call */
        (reprfunc)PP_ExClass1_str,                    /* tp_str */
        (getattrofunc)PP_ExClass1_getattro,                /* tp_getattro */
        (setattrofunc)PP_ExClass1_setattro,                /* tp_setattro */
        /* Functions to access object as input/output buffer */
        0,                              /* tp_as_buffer */
        /* Flags to define presence of optional/expanded features */
        Py_TPFLAGS_DEFAULT,             /* tp_flags */
        ExClass1__doc__,         /* tp_doc */
        /* Assigned meaning in release 2.0 */
        /* call function for all accessible objects */
        (traverseproc)0,                /* tp_traverse */
        /* delete references to contained objects */
        (inquiry)0,                     /* tp_clear */
        /* Assigned meaning in release 2.1 */
        /* rich comparisons */
        (richcmpfunc)0,                 /* tp_richcompare */
        /* weak reference enabler */
        0,                              /* tp_weaklistoffset */
        /* Added in release 2.2 */
        /* Iterators */
        (getiterfunc)0,                 /* tp_iter */
        (iternextfunc)0,                /* tp_iternext */
        /* Attribute descriptor and subclassing stuff */
        0,                             /* tp_methods */
        0,                              /* tp_members */
        0,                             /* tp_getset */
        0,                              /* tp_base */
        0,                              /* tp_dict */
        (descrgetfunc)0,                /* tp_descr_get */
        (descrsetfunc)0,                /* tp_descr_set */
        0,                              /* tp_dictoffset */
        (initproc)PP_ExClass1_init,                   /* tp_init */
        (allocfunc)PP_ExClass1_alloc,                  /* tp_alloc */
        (newfunc)PP_ExClass1_new,                    /* tp_new */
        (freefunc)PP_ExClass1_free,                   /* tp_free */
        (inquiry)0,                     /* tp_is_gc */
        0,                              /* tp_bases */
        0,                              /* tp_mro */
        0,                              /* tp_cache */
        0,                              /* tp_subclasses */
        0,                              /* tp_weaklist */
        (destructor)PP_ExClass1_del,                 /* tp_del */
        0,                              /* tp_version_tag */
#ifdef IS_PY3K
        (destructor)0,                  /* tp_finalize */
#endif
};

