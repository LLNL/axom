...
#            cpp_attrs = arg_call['attrs']
            ...
            if c_attrs['intent'] != 'out':  # inout or in
                if len_trim:
                    c_intent_in = arg_typedef.c_intent_in_trim
                else:
                    c_intent_in = arg_typedef.c_intent_in
                if c_intent_in:
                    fmt.cpp_var = 'SH_' + fmt.cpp_var
                    for intent_in in c_intent_in:
                        append_format(tmp_decl, intent_in, fmt)

            if arg_call:
                # convert C argument to C++
                append_format(call_list, arg_typedef.c_to_cpp, fmt)

            if c_attrs['intent'] != 'in':  # inout or out
                c_intent_out = arg_typedef.c_intent_out
                if c_intent_out:
                    fmt.var = fmt.cpp_var
                    fmt.cpp_val = wformat(arg_typedef.cpp_to_c, fmt)  # pick up c_str() from cpp_to_c
                    append_format(post_call, c_intent_out, fmt)
