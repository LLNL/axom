...
#                fmt.c_var_len = 'NNNN' + fmt.var
            ...
            if c_attrs['intent'] in [ 'inout', 'in']:
                if len_trim:
                    cmd_list = arg_typedef.c_pre_intent_in_trim
                else:
                    cmd_list = arg_typedef.c_pre_intent_in
                if cmd_list:
                    fmt.cpp_var = 'SH_' + fmt.cpp_var
                    for cmd in cmd_list:
                        append_format(pre_call, cmd, fmt)

                cmd_list = arg_typedef.c_post_intent_in
                if cmd_list:
                    for cmd in cmd_list:
            ...
            if c_attrs['intent'] in [ 'inout', 'out']:
                cmd_list = arg_typedef.c_pre_intent_out
                if cmd_list:
                    fmt.cpp_var = 'SH_' + fmt.cpp_var
                    fmt.var = fmt.cpp_var
                ...
                cmd_list = arg_typedef.c_post_intent_out
                if cmd_list:
                    fmt.var = fmt.cpp_var
                    fmt.cpp_val = wformat(arg_typedef.cpp_to_c, fmt)  # pick up c_str() from cpp_to_c
                    for cmd in cmd_list:
