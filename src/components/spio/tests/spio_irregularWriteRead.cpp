...
int main(int argc, char** argv)
{
  ...
  for (int f = 0; f < num_fields; ++f) {
    ...
    ostream << "fields" << f;
    DataGroup * flds = root->createGroup(ostream.str());
  ...
  for (int f = 0; f < num_fields; ++f) {
    std::ostringstream ostream;
    ostream << "fields" << f;
    DataGroup * flds = ds->getRoot()->getGroup(ostream.str());
    ...
    for (int g = 0; g < num_subgroups; ++g) {
      std::ostringstream gstream;
      gstream << "subgroup" << g;
      DataGroup * sg = flds->getGroup(gstream.str());
      ...
      if (g % 2) {
        ...
        DataView * view_orig = sg->getView(vstream.str());
        ...
        int testvalue = sg->getView(vstream.str())->getData();
