 ...
 * the InOutOctree representation and queries it over a uniform grid of a given
 *resolution
 ...
 *  /mesh_bounding_box  (array of 6 doubles: min_x, min_y, min_z, max_x, max_y,
 *max_z)
 *  /query_resolution   (array of 3 ints: i, j, k of query grid)
 *  /octree_containment (array of ints: one sample for each query point, with
 *value 0 or 1)
 *  /bvh_containment    (array of ints: one sample for each query point, with
 *value 0 or 1)
 ...
 *'--containment' command
 *  line option is present.  Similarly for the 'bvh_containment' and
 *'bvh_distance' and
 ...
 *   --mesh <file>                 (required) Surface mesh file (STL files are
 *currently supported)
 ...
 *(default: on)
 *   --[no-]containment            Indicates whether to test the point
 *containment (default: on)
